...
# for catgory in range(24):
# 	categorylinkBig.append(categorylinkAll[catgory])

# categoryRelativeLink = []

# for link in range(len(categorylinkBig)):
# 	categoryRelativeLink.append(categorylinkBig[link].get("href"))

# categoryCompreteLink = []

# for link in range(len(categoryRelativeLink)):
# 	categoryCompreteLink.append(urljoin(baseLink, categoryRelativeLink[link]))


# for categorylink in categoryCompreteLink:
# 	response = request.urlopen(categorylink)
# 	soup = BeautifulSoup(response)
# 	response.close()

# 	productList = soup.find('div', class_='product-list product-list--collection product-list--with-sidebar')
	
	
# 	itemNameLine = productList.find_all('a', class_='product-item__title text--strong link')
# 	itemImage = productList.find_all('img', class_='product-item__primary-image lazyload image--fade-in')
# 	itemPriceLine = productList.find_all('div', class_='product-item__price-list price-list')

# 	for name in range(len(itemNameLine)):
# 		itemPriceWithKanji = itemPriceLine[name].find_all('span', class_='tax')[1].text
# 		products.append({"JAN": "-", "Name": itemNameLine[name].text, "Image": 'https:'+(itemImage[name].get("data-src")).replace('{width}', '600'), "Prices": [int(re.findall('\(税込(.*)円\)', itemPriceWithKanji)[0].replace(',', ''))], "Places": ["ダイソー"]})

# 	pageCount = soup.find('span', class_='pagination__page-count').text
# 	numberOfPages = int(re.findall('(.*)ページ中.ページ目', pageCount)[0])

# 	for page in range(2, numberOfPages+1):
# 		response = request.urlopen(categorylink+'?page='+str(page))
# 		soup = BeautifulSoup(response)
# 		response.close()
...
# response_maruetsu = request.urlopen('https://maruetsu.net/shop/MAR_008/item_lst.php?mgcd=210')
# soup_maruetsu = BeautifulSoup(response_maruetsu)
# response_maruetsu.close()

# boxItemThumbBox = soup_maruetsu.find('div', class_='Box itemThumbBox')

# for row in products:
# 	print(row["JAN"])

#このプログラムで辞書リストを作って全ての店舗、製品を網羅し、一回でHTMLに書き出す。同じ製品の扱いなどの全工程をこちらで済ませる。
